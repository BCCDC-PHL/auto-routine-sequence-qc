# auto-routine-sequence-qc
Automated analysis of sequence data using the [BCCDC-PHL/routine-sequence-qc](https://github.com/BCCDC-PHL/routine-sequence-qc) pipeline.
...
```bash
auto-routine-sequence-qc --config config.json
```
...
```bash
auto-routine-sequence-qc --config config.json --log-level debug
```
...
```json
{
  ...
  "analysis_work_dir": "/path/to/auto-routine-sequence-qc-work",
  ...
  "pipelines": [
    {
      "pipeline_name": "BCCDC-PHL/routine-sequence-qc",
